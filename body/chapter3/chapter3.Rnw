...
# for(i in c(10, 20, 30)){
#   
#   aux_plotex_pred <- predict_tp(data = list_serie[[1]], tp = 0.02, sl = 0.025, h = i) %>%
#     mutate(class_2 = factor(class)) %>%
#     select(-one_of('class')) %>%
#     filter(year(timestamp) %in% seq(2009, 2010, 1)) %>% 
#     mutate(buy_close = if_else(class_2 == 'buy', close, NA_real_)) %>%
#     ggplot(aes(x = timestamp, y = close)) +
#     geom_line(colour = 'grey') +
#     geom_point(aes(y = buy_close), colour = 'darkgreen', size = 0.4) +
#     labs(title = paste('Horizonte de', i, 'períodos', sep = ' '), x = 'tiempo', y = 'precios de           cierre')
# 
#   list_plotex_pred %<>% rlist::list.append(aux_plotex_pred)
#   
# }
