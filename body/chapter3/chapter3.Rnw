...
La validación Cruzada es un metódo de validación y prueba que consiste en dividir los registros aleatoriamente en grupos de similar tamaño. El primer grupo es utilizado como validación y enel resto de los datos es ajustado el modelo. De cada grupo se obtienen resultados en nuestro caso el área bajo la cruva ROC (AUROC) y estos son promediados. Este metódo es utilizado dentro del modelo en cada uno de los períodos de entrenamiento.
